...
class Spot:
    ...
    def is_start(self):
        return self.color is ORANGE

    def is_end(self):
        return self.color is TURQUOISE
...
def heuristics(p1, p2):
    # p1, p2 = (row, col), (row, col); Called Manhattan Distance
    x1, y1 = p1
    x2, y2 = p2
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type is pygame.QUIT:
                ...
                elif spot is not end and spot is not start:
                    spot.make_barrier()
                ...
                if event.key is pygame.K_SPACE and not started and end and start:
                    for row in grid:
                        for spot in row:
